      ...
      <div className="domain-map-container">
        ...
        <FormControl style={{ width: "100%", marginTop: "16px" }} size="small">
          ...
          <Select
            ...
            value={updateD.mapto}
            onChange={(e) => setUpdateD(a => ({ ...a, mapto: e.target.value, mapstatus: true }))}
          >
            {
              labs?.map(a =>
                <MenuItem value={a?.deviceName}>{a?.deviceName}</MenuItem>
              )
            }
            ...
            {(folderOrPort.folder || folderOrPort.port) && < TextField
              ...
              onChange={(e) => {
                if (folderOrPort.folder) {
                  setUpdateD(a => ({ ...a, folder: e.target.value }))
                } else if (folderOrPort.port) {
                  setUpdateD(a => ({ ...a, port: e.target.value }))
                }
              }}
            />}
          </div>
