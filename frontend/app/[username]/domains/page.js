...
import { Divider, FormControl, IconButton, InputLabel, MenuItem, Select, TextField, Tooltip, Typography } from "@mui/material";
            ...
            <div className="domain-add-list">
              ...
              <div className="domains-list">
                {domains[0]?.domainList?.map((domain) => (
                  <div className="dot" key={domain.domainName} style={{
                    border: domain.mapstatus ? "1px solid var(--success)" : "1px solid var(--error)"
                  }}>
                    <div className="gap">
                        ...
                        <div className="domain-view">
                          <div className="domains-title">
                            <Typography variant="body" component="span">
                              <Link href={`https://${domain.domainName}`} target="_blank">
                                {domain.domainName}
                              </Link>
                            </Typography>
                            <Link href={`https://${domain.domainName}`} target="_blank">
                          ...
                          <div className="domain-status">
                            <Tooltip title={domain.mapto ? domain.mapto : "Not Mapped"} placement="top">
                              <Badge value={domain.mapto ? domain.mapto : "Not Mapped"} color={domain.mapstatus ? "success" : "error"} />
                            </Tooltip>
                            {/* {domain.mapstatus &&
...
const MapestheDomain = ({ details, setMaptheDomain, updatedomain, labs }) => {
  ...
  const [folderOrPort, setFolderOrPort] = useState({ port: false, folder: false });
  return (
    <div className="map-domain-to-lab">
      <div className="domain-map-container">
        <h3><b>{details.domainName}</b> map to the lab</h3>
        <FormControl style={{ width: "100%", marginTop: "16px" }} size="small">
          ...
          <Select
            ...
            value={updateD.mapto}
            onChange={(e) => setUpdateD(domain => ({ ...domain, mapto: e.target.value, mapstatus: true }))}
          >
            {
              labs?.map(domain =>
                <MenuItem value={domain?.deviceName}>{domain?.deviceName}</MenuItem>
              )
            }

          </Select>
        </FormControl>
        {updateD.mapto != null && <i>
          After successfull domain mapping your lab will automatically redeploy to make changes publicly
        </i>}
        {/* {
        ...
        <div className="map-domain-button">
          <Button value="map domain" color="success" onClick={() => {
          ...
          <Button onClick={() => setMaptheDomain({ status: false, details: null })} value="cancel" color="error"></Button>
        </div>
      </div>
    </div>
  );
